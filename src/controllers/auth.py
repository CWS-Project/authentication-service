...
async def create_user(user: NewUserRequest, response: Response):
    ...
    return make_response(response, 201 if success else 400, data=data)
...
async def login(request: Request, response: Response):
    ...
    return make_response(response, 200 if success else 400, data=data)
...
async def refresh_token(request: Request, response: Response):
    ...
    if not body:
        return make_response(response, 400, message="Request body not found")
    
    refresh_tok = body.get("refresh_token")
    if not refresh_tok:
        return make_response(response, 400, message="Refresh token not found")
