...
class DatabaseSession:
    ...
    def find(self, collection: str, condition: dict, num_rec: int = 10) -> Tuple[bool, list]:
        try:
            result = self._db.get_collection(collection).find(condition).limit(num_rec)
            if result is None:
                return False, []
    ...
    def findOne(self, collection: str, condition: dict) -> Tuple[bool, dict | None]:
        try:
            result = self._db.get_collection(collection).find_one(condition)
            if result is None:
                return False, None
            return True, result
        except Exception as e:
